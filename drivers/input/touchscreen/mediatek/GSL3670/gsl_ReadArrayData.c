...
static short dac_save[4][24];
static short dac_data[4][24];
...
//	memset(test_result,'\0',sizeof(test_result));
	...
	printk("GSL:: drv_num = %d,sen_num = %d\n",drv_num,sen_num);
	for (i=0;i<drv_num;i++)
	{
		for (j=0;j<sen_num;j++)
		{
...
#if 0
int gsl_obtain_array_data_ogv(int *gsl_ogv_new,int i_max,int j_max)
{
	int i,j;
	unsigned int ret = 1;
	...
	for(i=0;i<i_tmp;i++){
		for(j=0;j<j_tmp;j++){
		gsl_ogv_new[i*14+j]=(int)gsl_ogv[i][j];
		printk("%4d ",gsl_ogv_new[i*14+j]);
...
#endif
int gsl_obtain_array_data_dac(unsigned short *dac,int i_max,int j_max)
{
	int i;
	unsigned int ret = 1;
	int j_tmp = (j_max > 24 ? 24 : j_max);
	...
	for(i=0;i<i_tmp;i++){
		memcpy(&dac[i*j_max],dac_data[i],j_tmp*(sizeof(unsigned short)));
	}
	return ret;
}
